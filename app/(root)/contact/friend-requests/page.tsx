...
const Page = () => {
  ...
  async function fetchFriendRequests() {
    fetch(`${apiBaseUrl}/contact/requests?userId=${userId}`)
  ...
  async function fetchSentFriendRequests() {
    fetch(`${apiBaseUrl}/contact/sent?userId=${userId}`)
      .then((response) => response.json())
      .then((data) => {
        if (data.success) {
  ...
  async function handleDeny(senderId: string) {
    try {
      const response = await fetch(
        `${apiBaseUrl}/contact/deny?userId=${userId}&senderId=${senderId}`,
        {
          method: "POST",
          headers: {
      ...
      if (data.success) {
        ...
        showNotification(
          "Failed to deny friend request: " + (data.message || "Unknown error"),
          "error"
        );
      }
    } catch (error) {
      ...
      showNotification(
        "An error occurred while denying the friend request",
        "error"
      );
    }
  }
  ...
  async function handleAccept(senderId: string) {
    try {
      const response = await fetch(
        `${apiBaseUrl}/contact/accept?userId=${userId}&senderId=${senderId}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...
            >
              <div className="flex items-start space-x-3">
                <img
                  src={
                    f.senderImage ||
                    `https://ui-avatars.com/api/?name=${f.senderName}`
                  }
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-500">
                    {f.createdAt ? f.createdAt.split("T")[0] : "Unknown date"} -
                    From phone number
                  </div>
                  <div className="mt-2 text-sm">
                    Hi there, i'm {f.senderName}. Wanna be friends?
                  </div>
                  <div className="mt-3 flex space-x-2">
                    <button
                      onClick={() => handleAccept(f.senderId)}
            ...
            >
              <div className="flex items-center space-x-3">
                <img
                  src={
                    f.receiverImage ||
                    `https://ui-avatars.com/api/?name=${f.receiverName}`
                  }
                  className="w-10 h-10 rounded-full object-cover"
                  alt={f.receiverName}
                />
                <div>
                  ...
                  <div className="text-sm text-gray-500">Request sent</div>
                </div>
              </div>
